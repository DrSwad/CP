...
template <class T>
bool operator<(Fraction<T> p, Fraction<T> q) {
...
template <typename T>
ll sumv(std::vector<T> &v) {
  return std::accumulate(all(v), 0LL);
}

template <typename T>
T minv(std::vector<T> &v) {
  return *std::min_element(all(v));
}

template <typename T>
T maxv(std::vector<T> &v) {
...
int main() {
  ...
  while (true) {
    int x = rand(0, n - 1), y = (x + 1, n);
    if (x == y) {
      continue;
    }
    if ((v[x] & v[y]).count() >= n / 2) {
      printf("%d %d\n", x + 1, y + 1);
      return 0;
    }
  }
