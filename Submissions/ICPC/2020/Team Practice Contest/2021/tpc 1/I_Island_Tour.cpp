...
void go(int id1, int st1, int id2, int st2) {
  ...
  for (int at1 = st1, at2 = st2; (!moved1 or at1 != st1) and (!moved2 or at2 != st2); ) {
    if (at1 == at2 and done1 < t[id1][at1] and done2 < t[id2][at2]) {
      return void(poss[dpi][st1][st2] = false);
    }
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = i + 1; j < 3; j++) {
        if (~at[i] and at[i] == at[j]) {
          // debug() << name(i) name(j) name(at[i]) name(at[j]);
          return T;
        }
      }
    }
