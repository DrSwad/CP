...
vector<Mint> multiply(vector<Mint> p1, vector<Mint> p2) {
  int d1 = (int)p1.size() - 1;
  int d2 = (int)p2.size() - 1;
  vector<Mint> res(d1 + d2 + 1, 0);
...
void prepare(int n) {
  ...
  int r = bigMod(G, (md - 1) / n, md);
  inv_n = bigMod(n, md - 2, md), w[0] = w[n] = 1;
  for (int i = 1; i < n; ++i) w[i] = (ll) w[i - 1] * r % md;
  ...
  for (int m = 2; m <= n; m <<= 1) {
    for (int i = 0; i < n; i += m) {
      for (int j = 0; j < (m >> 1); ++j) {
        ...
        int t = (ll) v * w[dir ? n - n / m * j : n / m * j] % md;
        v = u - t < 0 ? u - t + md : u - t;
        u = u + t >= md ? u + t - md : u + t;
      }
    }
  } if (dir) for (int i = 0; i < n; ++i) a[i] = (ll) a[i] * inv_n % md;
}
...
vector<int> multiply(vector<int> a, vector<int> b) {
  ...
  for (int i = 0; i < sz; ++i) f_a[i] = (ll) f_a[i] * f_b[i] % md;
...
void test_case() {
  ...
  for (int i = 0; i <= n; i++) {
    if (i <= 1 or i % 2 == 0 or i > k) coeff[i] = 0;
    else coeff[i] = Mint(1) / (i * fact[(i - 1) / 2] * fact[(i - 1) / 2]);
  }
  ...
  function<void(int, int)> solve =
    [&](int L, int R) {
      ...
      int mid = (L + R) / 2;
      solve(L, mid);
      vector<int> oldDP(dp.begin() + L, dp.begin() + mid + 1);
      vector<int> curCoeff(coeff.begin() + 0, coeff.begin() + (R - L + 1));
      vector<int> mult = multiply(oldDP, curCoeff);
