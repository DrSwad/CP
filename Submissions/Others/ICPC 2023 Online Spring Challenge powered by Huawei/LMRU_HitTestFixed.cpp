...
namespace ReplacementAlgorithm {
  ...
  int LRU_fault_remaining = 1000;
  int LRU_hits = 0;
  int MRU_fault_remaining = 1000;
  int MRU_hits = 0;
  bool LRU_testing = false;
  bool MRU_testing = false;
  bool LRU_final_choice = false;

  void initialize() {
    LRU_fault_remaining = min(LRU_fault_remaining, int(total_buffer_size * 1.5));
    MRU_fault_remaining = min(MRU_fault_remaining, int(total_buffer_size * 1.5));
  }
  ...
  int handle_operation(int tenant, int page) {
    ...
    if (page_position_in_buffer[tenant][page] != 0) {
      ...
      if (buffer_positions_used >= total_buffer_size) {
        if (LRU_testing) LRU_hits++;
        if (MRU_testing) MRU_hits++;
      }
    ...
    if (buffer_positions_used >= total_buffer_size) {
      if (LRU_testing and LRU_fault_remaining == 0 and MRU_fault_remaining) {
        LRU_testing = false;
        MRU_testing = true;
      }
      if (MRU_testing and MRU_fault_remaining == 0) {
        MRU_testing = false;
        if (LRU_hits >= MRU_hits) LRU_final_choice = true;
        else LRU_final_choice = false;
      }
      if (LRU_testing) LRU_fault_remaining--;
      if (MRU_testing) MRU_fault_remaining--;
    }

    if (tenant_current_buffer_size[tenant] >= buffer_max_sizes[tenant]) {
      // cerr << "Page must replace own tenant's page" << endl;
      int RU_page = get_RU_page(tenant, LRU_testing or LRU_final_choice);
      assert(RU_page != 0);
      ...
      if (buffer_positions_used < total_buffer_size) {
        ...
        bool LRU = LRU_testing or LRU_final_choice;
