...
namespace ReplacementAlgorithm {
  ...
  bool Custom_compare(const Node& node1, const Node& node2) {
    if (!node1.active) return false;
    if (!node2.active) return true;
    return make_pair(-current_algorithm->tenant_current_buffer_size[node1.tenant_id], priorities[node1.tenant_id]) < make_pair(-current_algorithm->tenant_current_buffer_size[node2.tenant_id], priorities[node2.tenant_id]);
  }
  ...
  void initialize() {
    ...
    current_algorithm_index = CUSTOM_INDEX;
  ...
  void force_verdict(string verdict) {
    if (verdict == "WA") {
      cout << "0" << endl;
    }
    else if (verdict == "TLE") {
  ...
  int depth;
  void check_within_segment(int &L, int &R, int value) {
    if (total_operations < 100) depth = -1;
    if (depth < 0) return;
    ...
    if (depth > 0) {
      for (int i = 0; i < 6; i++) {
        ...
        if (l <= total_operations and total_operations <= r) {
          L = l;
          R = r;
          break;
        }
    ...
    for (int i = 0; i < 6; i++) {
      if (L + segment * i <= total_operations and total_operations < L + segment * (i + 1)) {
        ReplacementAlgorithm::force_verdict(verdict_order[i]);
        break;
      }
    }
  }
  ...
  int handle_operation(int tenant, int page) {
    ...
    int algo_index = CUSTOM_INDEX;

    if (current_algorithm_index != algo_index) {
...
int main() {
  ...
  ReplacementAlgorithm::depth = 7;
  int L = 1, R = MAX_OPERATIONS;
  while (ReplacementAlgorithm::depth >= 0) {
    ReplacementAlgorithm::check_within_segment(L, R, total_operations);
  }
  ...
  for (int operation_id = 1; operation_id <= total_operations; operation_id++) {
    ...
    cout << ReplacementAlgorithm::handle_operation(tenant, page);
    ReplacementAlgorithm::flush_out();
  } 
