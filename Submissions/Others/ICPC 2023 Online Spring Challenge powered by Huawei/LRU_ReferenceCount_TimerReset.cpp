...
namespace ReplacementAlgorithm {
  ...
  const int max_page_reference_count = 8;

  int refill_timer_amount;
  int timer = 0;
  vector<pair<int, int>> pages_accessed_within_time_period;

  void initialize() {
    refill_timer_amount = max(1, (int)sqrt(total_operations) * 10);
  }
  ...
  int handle_operation(int tenant, int page) {
    ...
    if (timer == 0) {
      timer = refill_timer_amount;
      for (auto [tenant, page] : pages_accessed_within_time_period) {
        page_reference_count[tenant][page] = max(0, page_reference_count[tenant][page] - 1);
      }
      pages_accessed_within_time_period.clear();
    }
    else timer--;

    pages_accessed_within_time_period.emplace_back(tenant, page);
