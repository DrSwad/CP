...
namespace ReplacementAlgorithm {
  ...
  const int min_window_size_for_switch = 1000;
  const vector<vector<double>> algo_resistance = {
    {1000, 200, 27},
    {30, 1000, 30},
    {30, 30, 1000}
  };
  ...
  int max_lru_to_lru_ref_back_delay = 20000;
  ...
  bool should_stop_lru() {
    return !mru_switch_remaining or (lru_switch_remaining and max_lru_to_lru_ref_back_delay <= 0) or (!lru_switch_remaining and current_algorithm_index != LRU_INDEX);
  }

  bool should_stop_lfu() {
    return !mru_switch_remaining or (!lfu_switch_remaining and current_algorithm_index != LFU_INDEX);
  }

  bool should_stop_mru() {
    return !lru_switch_remaining or !lfu_switch_remaining or !mru_switch_remaining or max_mru_delay <= 0;
  }

  bool should_stop_lru_ref() {
    return should_stop_lru() and should_stop_lfu() and should_stop_mru();
  }

  bool should_stop_algo(int algo_index) {
    return (algo_index == LRU_REF_INDEX and should_stop_lru_ref()) or
    (algo_index == LRU_INDEX and should_stop_lru()) or
    (algo_index == LFU_INDEX and should_stop_lfu()) or
    (algo_index == MRU_INDEX and should_stop_mru());
  }
  ...
  int handle_operation(int tenant, int page) {
    ...
    for (int algo_index = 0; algo_index < 4; algo_index++) {
      if (should_stop_algo(algo_index)) continue;
