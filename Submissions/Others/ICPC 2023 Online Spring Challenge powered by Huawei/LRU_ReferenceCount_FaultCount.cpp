...
namespace ReplacementAlgorithm {
  ...
  const int max_page_reference_count = 2;

  int tenant_fault_count[MAX_TENANTS + 1];
  double tenant_fault_weight[MAX_TENANTS + 1];

  void initialize() {
    int total_base_sizes = 0;
    for (int t = 1; t <= total_tenants; t++) {
      total_base_sizes += buffer_base_sizes[t];
    }
    for (int t = 1; t <= total_tenants; t++) {
      tenant_fault_weight[t] = 1 - (double)buffer_base_sizes[t] / total_base_sizes;
    }
  }
  ...
  int get_LRU_page(int tenant) {
    while (true) {
      ...
      auto [page, operation_id] = queue_of_tenant_pages[tenant].front();
      if (page_reference_count[tenant][page] == 0) return page;
      else {
        queue_of_tenant_pages[tenant].emplace(queue_of_tenant_pages[tenant].front());
        queue_of_tenant_pages[tenant].pop();
  ...
  int handle_operation(int tenant, int page) {
    ...
    if (tenant_current_buffer_size[tenant] >= buffer_max_sizes[tenant]) {
      ...
      if (buffer_positions_used < total_buffer_size) {
        ...
        double least_cost = HUGE_VAL;
        pair<int, int> least_cost_page;
        ...
        for (int t = 1; t <= total_tenants; t++) {
          ...
          int dist = operations_processed - page_last_access[t][LRU_page];
          double tenant_weighted_cost = tenant_fault_weight[t] * tenant_fault_count[t];
          double page_cost = 3 * tenant_weighted_cost * priorities[t] - dist / 2.0;

          if (page_cost < least_cost) {
            least_cost = page_cost;
            least_cost_page = make_pair(t, LRU_page);
          }
        }

        assert(least_cost < HUGE_VAL);

        auto [LRU_tenant, LRU_page] = least_cost_page;
