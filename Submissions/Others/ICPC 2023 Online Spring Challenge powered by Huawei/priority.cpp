  ...
  int handle_operation(int tenant, int page) {
    ...
    if (tenant_current_buffer_size[tenant] >= buffer_max_sizes[tenant]) {
      ...
      if (buffer_positions_used < total_buffer_size) {
        ...
        int lowest_bad_priority = MAX_BAD_PRIORITY + 1;
        ...
        for (int t = 1; t <= total_tenants; t++) {
          ...
          if (make_pair(bad_priorities[t], page_last_access[t][LRU_page]) < make_pair(lowest_bad_priority, earliest_access_time)) {
            lowest_bad_priority = bad_priorities[t];
